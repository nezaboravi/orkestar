...
export async function chooseTeam({ catalog, primary, question, write, confirm = true }) {
  ...
  for (const role of teamRoles) {
    ...
    const picked = await searchableChoice({items:choices,label:item => `${item.harness} / ${item.model}`,recommended,question,write,title:titles[role]});
    const route = picked.item;
    ...
    const effort = await chooseEffort({levels,fallback,question,write});
    team[role] = { harness: route.harness, model: route.model, effort, mode: picked.auto ? 'auto' : 'manual' };
  }
