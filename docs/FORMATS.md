...
- Contains: truthful `DONE`, `PARTIAL`, or `FAILED` status, every root and child session, actual agent name, actual provider/model, exact OpenCode input/output/reasoning tokens, exact recorded cost, verification evidence, and blockers
